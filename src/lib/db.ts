...
const cnt = (db.prepare('SELECT COUNT(*) as c FROM feature_requests').get() as { c: number }).c;
...
if (cnt === 0) {
  ...
  const accounts: [string,string,string,string,string,string,string,string][] = [
  ...
  const getPriority = (rev: number, mentions: number, accts: number): string => {
  ...
  const insertCV = db.prepare('INSERT INTO customer_validations (id, request_id, status) VALUES (?, ?, ?)');

  interface FRRow { title:string; area:string; status:string; accountStr:string; mentions:number; revStr:string; stageOverride?: string; }
  ...
  const insertRABatch = db.transaction((reqId: string, names: string[]) => {
    for (const n of names) insertRA.run(crypto.randomUUID(), reqId, n.trim());
  });

  const insertStages = db.transaction((reqId: string, finalStage: string) => {
  ...
  for (const row of rows) {
    ...
    const rawFeedback = `"We really need ${row.title.toLowerCase()}." - Reported by ${accountNames.slice(0,3).join(', ')}${acctCount > 3 ? ` and ${acctCount - 3} more accounts` : ''}. Total ${row.mentions} mentions, ${acctCount} accounts, estimated revenue impact ${row.revStr}.`;
  ...
  const allReqs = db.prepare('SELECT id, product_area FROM feature_requests').all() as {id:string;product_area:string}[];
  const insertRelated = db.prepare('INSERT OR IGNORE INTO related_feedback (id, request_id, related_request_id) VALUES (?, ?, ?)');
  const linkRelated = db.transaction(() => {
